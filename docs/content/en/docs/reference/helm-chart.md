...
| rthooks.failAllowNamespacesPatterns | list | `[]` | List of glob patterns for namespaces to allow Pod creation for, in case tetragon-oci-hook fails to reach Tetragon agent. Supports a single '*' wildcard: 'kube-*' (prefix), '*-system' (suffix), 'foo-*-bar' (contains), or '*' alone to match any namespace. Can be combined with failAllowNamespaces. Ignored if extraHookArgs sets fail-cel-expr. Example: ["acme-*", "dev-*"] |
